...
class News
{
    ...
    public static function get_list_data($number_page)
    {
        ...
        $lang = SiteFunction::getLang();
        $data = [];
        $result = $db->query("SELECT `post_title`, `short_desc`, `permalink`, `data_field`, `img` FROM " . self::NAME_DB . " where lang='" . $lang . "' AND status=1 ORDER BY data DESC LIMIT " . self::SHOW_BY_DEFAULT_CATEGORI . " OFFSET " . self::SHOW_BY_DEFAULT_CATEGORI * $number_page);
        $i = 0;
        while ($row = $result->fetch()) {
    ...
    public static function getDataLastPosts($lang, $number)
    {
        $db = Db::getConnection();
        $data = [];
        $result = $db->query("SELECT post_title, permalink, short_desc, data_field, img FROM " . self::NAME_DB . " WHERE status='1'" . " AND lang='" . $lang . "'" . " ORDER BY data DESC LIMIT " . $number);
        $i = 0;
        while ($row = $result->fetch()) {
            $data['post_title'][$i] = $row['post_title'];
            $data['permalink'][$i] = $row['permalink'];
    ...
    public static function getUniqueDirectionId($lang)
    {
        $db = Db::getConnection();
        $data = [];
        $result = $db->query("SELECT direction_id FROM " . self::NAME_DB . " WHERE status='1'" . " AND lang='" . $lang . "'");
        $data['direction_id'] = [];
        while ($row = $result->fetch()) {
            if ($row['direction_id'] !== "[]") {
