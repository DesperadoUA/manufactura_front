...
<main class="news_page_template">
    <section class="section_1 padding_first_section">
        <div class="container">
            <div class="row">
                <div class="col-lg-12">
                    <h1><?php echo translate("Новости", "Новини", "News"); ?></h1>
                </div>
                <div class="col-lg-12">
                                ...
                                <ul>
                                    ...
                                    if (count($data_direction) != 0) {
                                        for ($i = 0; $i < count($data_direction['post_title']); $i++) {
                                            echo '<li class="item_select_found" 
                                            data-type="news-direction" data-id="' . $data_direction["id"][$i] . '">' . $data_direction["post_title"][$i] . '</li>';
                                        }
                                    }
                                    ?>
                                </ul>
            ...
            <div class="row category_news_wrapper">
                ...
                if (count($data_news) != 0) {
                    for ($i = 0; $i < count($data_news['permalink']); $i++) {
                                ...
                                <div class='litle_card'>
                                    ...
                                    <div class='litle_card_right'>
                                        ...
                                        <p class='litle_card_text_first color_red'>
                                           {$data_news['data_field'][$i]}
                                         </p>
                                        <p class='litle_card_text_second color_blue'>
                                           {$data_news['post_title'][$i]}
                                        </p>
                                        <p>{$data_news['short_desc'][$i]}</p>
                                        <p class='litle_card_read_more'>
                                           <a href='{$data_news['permalink'][$i]}'>{$button_text}</a>
                                        </p>
                                        </div>
                                    </div>
                                </div>
